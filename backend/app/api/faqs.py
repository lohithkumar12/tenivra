...
def list_faqs(db: Session = Depends(get_db), user: User = Depends(require_clinic_admin)):
...
def create_faq(body: FAQCreate, db: Session = Depends(get_db), user: User = Depends(require_clinic_admin)):
...
def update_faq(
    faq_id: str, body: FAQUpdate, db: Session = Depends(get_db), user: User = Depends(require_clinic_admin)
):
...
def delete_faq(faq_id: str, db: Session = Depends(get_db), user: User = Depends(require_clinic_admin)):
    faq = db.query(FAQ).filter(FAQ.id == faq_id, FAQ.tenant_id == user.tenant_id).first()
    if not faq:
        raise HTTPException(status_code=404, detail="FAQ not found")
    db.delete(faq)
